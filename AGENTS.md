...
- `.github/workflows/publish.yml` publishes stable GitHub Releases whose `v<version>` tag matches the manifest, after running all tests. It requires the repository secret `VSCE_PAT`. See README.md for setup and the announced global PAT retirement deadline. Check the pinned CLI's `publish --help` before changing authentication: upstream `vsce` documentation describes `--oidc`, but the verified 4.0.0 release does not support it.
