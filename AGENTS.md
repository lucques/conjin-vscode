...
- `.github/workflows/publish.yml` runs on pushed `v*` tags and requires a stable version matching the manifest. After all tests pass, it publishes to Marketplace and creates the GitHub Release with the VSIX attached in the same run. Keep both operations in this workflow: releases created with `GITHUB_TOKEN` do not trigger a separate release-event workflow. Marketplace access uses `VSCE_PAT`; GitHub Release creation uses the built-in token with `contents: write`. Use a new version and tag when changing a tagged release; never move a published tag. See README.md for setup, rerun behavior, and the announced global PAT retirement deadline. Check the pinned CLI's `publish --help` before changing authentication: upstream `vsce` documentation describes `--oidc`, but the verified 4.0.0 release does not support it.
